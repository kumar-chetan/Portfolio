...
// Import logo images for different themes
import lightLogoImage from './image.png'; // Replace with your light mode logo
import darkLogoImage from './1-removebg-preview.png';   // Replace with your dark mode logo
...
export default function Header({ logo }: { logo: string }) {
    ...
    // Determine the logo image based on the current theme
    const logoImage = theme === 'dark' ? darkLogoImage : lightLogoImage;
    // Extract the last name from the logo
    const lastName = logo.split(' ').pop(); 
    ...
    return (
        <header className={`backdrop-filter backdrop-blur-lg ${scroll ? 'border-b bg-white bg-opacity-40' : 'border-b-0'} dark:bg-grey-900 dark:bg-opacity-40 border-gray-200 dark:border-b-0 z-30 min-w-full flex flex-col fixed`}>
            <nav className='lg:w-11/12 2xl:w-4/5 w-full md:px-6 2xl:px-0 mx-auto py-4 hidden sm:flex items-center justify-between'>
                <Link href={'/'} className='flex items-center gap-2 hover:text-violet-700 hover:dark:text-violet-500 transition-colors duration-300'>
                    <Image 
                        src={logoImage} 
                        alt="Logo" 
